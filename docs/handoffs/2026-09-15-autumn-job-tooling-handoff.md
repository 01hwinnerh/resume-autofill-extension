...
当前 HEAD：`e6e376917204b175e2cd1002e2761136e5612f8c`，最近一次提交标题为 `docs(resume-autofill): replace interview plan with debrief bot scope`。当前分支比 `origin/aime/1789464099-profile-mapping-workflow` 超前 6 个提交。

当前工作区在本次交接前是干净的，当前分支已跟踪 `origin/aime/1789464099-profile-mapping-workflow`。不要假设它仍然是早期的 `feat/resume-autofill-mvp` 分支；`d9dec1a` 及其后续提交已经重构资料管理、映射作用域、扫描逻辑、UI 入口、投递记录和 ByteDance Jobs 适配。
...
- 侧边栏：处理当前活动页面的扫描、匹配、确认、填写、资料快速编辑、自定义字段、映射管理和投递记录入口。
- Options 页面：维护完整个人资料，支持多段教育、工作和项目经历。
- 预览/投递记录页面：展示本次填写预览、个人资料预览，以及用户手动确认后的投递记录和阶段时间线。
...
- `src/shared/application-record.ts` 定义投递记录、阶段事件和预设阶段；
- `src/storage/application-store.ts` 负责本地记录、阶段时间线和写入队列；
- `entrypoints/applications/` 提供记录列表、搜索、详情、阶段追加、备注和删除；
- 填写预览页和侧边栏支持用户在真实提交后手动记录本次申请；
- 当前不会根据填写完成或检测到提交按钮自动判断“已投递”。

当前已加入 `ByteDanceJobsAdapter`，只匹配 `jobs.bytedance.com` 的简历申请路径，并提供基础字段和多段教育、工作、项目经历映射。
...
| 检查 | 结果 |
| --- | --- |
| `npm test` | 当前 HEAD：30 个测试文件、119 个测试通过 |
| `npm run typecheck` | 通过 |
| `npm run build` | 通过，生成 Chrome MV3 包 |
| `npm run e2e` | 当前 HEAD：12 个本地 Chrome E2E 测试通过 |

当前 E2E 覆盖基础控件、综合控件、受控输入、动态区域、重复经历、已有值保护、文件/CAPTCHA 边界、失败隔离、手动提交边界、日期校验、资料映射和投递流程相关页面。
...
| `entrypoints/sidepanel/App.tsx` | 侧边栏视图切换、扫描、资料、映射、填写和投递记录流程 |
| `entrypoints/sidepanel/ReviewPanel.tsx` | 扫描结果预览、字段选择、填写确认和定位页面字段 |
| `entrypoints/options/App.tsx` | 完整资料中心和多段经历管理 |
| `entrypoints/preview/` | 本次填写/资料预览和用户确认后的投递记录入口 |
| `entrypoints/applications/` | 投递记录列表、详情和阶段时间线 |
...
- 更多真实招聘平台 Adapter；当前只有 ByteDance Jobs Adapter，其他平台仍依赖通用表单匹配。
- Chrome Web Store 发布；当前构建可以作为本地解压扩展加载，但还没有商店图标、截图、隐私声明和完整发布材料。
- 投递工作台已实现岗位记录、阶段时间线、备注和基础搜索；截止时间、简历版本关联、提醒、统计和批量管理尚未实现。
- 面试复盘辅助机器人尚未实现；当前已确认 RAG 为核心能力，方向性计划见 `docs/superpowers/plans/2026-09-15-interview-debrief-bot-initial-plan.md`。
- 文件上传、开放题答案库、多份独立简历版本和 AI 均不在当前代码中。
...
4. 完善截止时间、简历版本和提醒等投递工作台能力。
5. 选择用户最常投递的其他真实平台，单独设计并实现 Adapter。
6. 另行设计面试复盘辅助机器人，不把语音处理、知识库和面试训练逻辑塞进表单 runtime。
...
## 8. 2026-09-15 追加确认：扫描绑定、全屏预览与投递记录

### 8.1 新发现的基本可用性问题

用户在真实招聘页面测试时发现：侧边栏仍显示或扫描本地测试页。当前实现的根因是扫描、填写、定位各自重新查询活动标签页，`ScanResult` 未绑定明确 `tabId`；标签页切换或 URL 变化后旧扫描结果也不会失效。后续修复必须：

- 扫描时确定明确 tabId；
- `ScanResult` 保存 tabId、URL、标题和扫描时间；
- 填写和定位使用扫描会话中的 tabId，不再临时改用其他活动页；
- 标签页切换、刷新或 URL 变化后使旧结果失效；
- 填写前核对 tabId 与 URL，不一致时阻止操作；
- 侧边栏明确展示当前目标页面。

### 8.2 已确认的全屏预览方案

当前 Chrome Options 弹窗和侧边栏均过窄，用户明确要求大屏预览。已确认同时提供：

1. **本次填写预览**：页面字段、当前值、将填值、资料来源、置信度、确认原因、段落和定位操作；
2. **个人资料预览**：按基本信息、教育、工作、项目、技能和求职意向展示可读结果。

实现方向：扩展内部独立标签页 + 临时 preview session；不能把个人资料直接编码到 URL。空选择、扫描失效等情况必须显示明确空状态和恢复操作。完整资料中心也应在新标签页打开，而不是 Chrome 小型 Options 弹窗。

### 8.3 已确认的投递记录 MVP

用户在真实页面手工提交成功后，主动点击“已完成投递，记录本次申请”；自动填写完成不能自动认定投递成功。

首版记录：公司、岗位、岗位 URL、来源网站、地点、投递时间、当前阶段、备注和阶段时间线。阶段预设：准备投递、已投递、在线测评、笔试、一面、二面、三面/终面、HR 面、Offer、已拒绝、已结束、已撤回；允许自定义阶段。每次阶段变化追加带时间戳事件，可修改时间、备注和删除误操作。

侧边栏仅保留当前岗位快捷入口，大屏工作台负责记录列表、详情与时间线。

### 8.4 最新强制工作流

用户明确要求：在达到基本可用并由用户决定之前，不得继续 push，也不得创建 PR。后续必须遵循：

```text
本地开发 → 自动化测试 → 用户本地验收 → 根据反馈继续修复 → 用户明确确认 → push → PR
```

当前先只在本地修复“活动页面扫描绑定 + 大屏预览”，通过用户验收后再进入投递记录 MVP。所有开发继续串行进行，不派发智能体，不创建额外 worktree。
