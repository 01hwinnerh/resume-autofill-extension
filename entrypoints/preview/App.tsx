...
export default function App() {
  ...
  async function runFill(requestedFields: ConfirmedFill[]) {
    ...
    if (summary.failed.length > 0) { setMessage(`${summary.verified.length} 项成功，${summary.failed.length} 项需要处理；已自动定位第一项。`); await locateFailure(summary.failed[0].fieldId); return; }
  ...
  async function locateFailure(fieldId: string) {
    ...
    if (current?.kind !== 'fill' || !current.target) return;
    await browser.tabs.update(current.target.tabId, { active: true });
    const response = await browser.runtime.sendMessage({ type: 'focus-active-field', fieldId, target: current.target }) as RuntimeCommandResponse;
    if (!response.ok || !('focused' in response.data) || !response.data.focused) setMessage(response.ok ? '页面中已找不到该字段，请重新扫描后再定位。' : runtimeErrorMessage(response.error, '定位失败'));
  }
  ...
  return <main className="preview-shell">
    ...
    {active === 'profile' ? <ProfileView sections={profileSections} /> : session.kind === 'fill' && <>{session.items.length === 0 ? <div className="empty-state"><h2>当前没有可预览的字段</h2><p>请返回侧边栏，重新扫描页面并至少选择一个可填写字段。</p><button type="button" onClick={() => void returnToTarget()}>返回招聘页</button></div> : <div className="preview-layout">
      <aside><strong>填写摘要</strong><span>{session.items.length} 个字段</span><span>{new Set(session.items.map((item) => item.group)).size} 个分组</span>{modifiedFieldIds.size > 0 && <span className="modified-count">本次已修改 {modifiedFieldIds.size} 项</span>}<small>修改会自动保存到本地资料；离开或填写前会等待保存完成。</small><small>最终提交始终由你在招聘页面完成。</small></aside>
