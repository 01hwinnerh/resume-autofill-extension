  ...
  return <div className="profile-preview">{sections.map((section) => (
    <section className="preview-section" key={section.id}>
      ...
      <div className="profile-grid">{section.items.map((item) => (
        <div className="profile-item" key={item.key}><span>{item.label}</span><strong>{item.value || '空'}</strong></div>
      ))}</div>
    </section>
  ))}</div>;
}
...
export default function App() {
  ...
  return <main className="preview-shell">
    <header className="preview-header">
      ...
      {session.kind === 'fill' && <div className="header-actions"><button className="secondary" onClick={() => void returnToTarget()}>返回招聘页</button></div>}
    </header>
    ...
    {active === 'profile' ? <ProfileView sections={profileSections} /> : session.kind === 'fill' && <>
      {session.items.length === 0 ? <div className="empty-state"><h2>当前没有可预览的字段</h2><p>请返回侧边栏，重新扫描页面并至少选择一个可填写字段。</p><button onClick={() => void returnToTarget()}>返回招聘页</button></div> : <div className="preview-layout">
        ...
        <div className="preview-content">{groups.map(([group, items]) => <section className="preview-section" key={group}><header><h2>{group}</h2><span>{items.length} 项</span></header><div className="comparison-table"><div className="table-head"><span>页面字段</span><span>当前值</span><span>将填写值（可修改）</span><span>依据</span></div>{items.map((item) => <div className={`comparison-row${modifiedFieldIds.has(item.fieldId) ? ' modified' : ''}`} key={item.fieldId}><strong>{item.label}</strong><span>{item.currentValue || '空'}</span><span><input className="edit-next-value" aria-label={`修改${item.label}的待填值`} type="text" value={item.nextValue} onChange={(event) => updateDraft(item.fieldId, event.target.value)} onBlur={(event) => void persistEditedValue(item.fieldId, event.currentTarget.value)} /><small>{modifiedFieldIds.has(item.fieldId) ? '已修改，失焦后同步到本地资料' : '失焦后同步到本地资料'}</small></span><span><b>{item.confidence === undefined ? '手动' : `${Math.round(item.confidence * 100)}%`}</b><small>{item.reasons.join('；') || item.source}</small></span></div>)}</div></section>)}</div>
      </div>}
    </>}
